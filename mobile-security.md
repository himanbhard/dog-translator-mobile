...
### iOS Simulator Setup
When running on the iOS Simulator, `DeviceCheck` and `AppAttest` are not available. You must use the **Debug Provider**.

1.  **Configure `appCheck.ts`**:
    Ensure the provider is set to `'debug'` when `__DEV__` is true.
    ```typescript
    apple: {
...
2.  **Get the Debug Token**:
    Run the app in the simulator and check the native logs (using `xcrun simctl spawn booted log stream` or Xcode console). Look for:
    `[AppCheckCore] App Check debug token: 'UUID'`

3.  **Register in Firebase Console**:
    *   Go to **App Check > Apps > (your iOS app)**.
    *   Click the **3 dots menu > Manage Debug Tokens**.
    *   Add the token found in the logs.

Failure to do this will result in `401 Unauthorized: Missing App Attestation` errors.
